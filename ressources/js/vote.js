...
function vote_recipe(number){
    ...
    request.open("GET", `https://cookit.ovh/ressources/api/api_vote.php?action=2&vote=${number}&user=${id_user.innerText}&token=${token.innerText}&recipe=${id_recipe}`);
    request.send();
...
function getVote(recipe){
    ...
    request2.open("GET", `https://cookit.ovh/ressources/api/api_vote.php?action=1&recipe=${id_recipe}`);
    request2.send();
}
